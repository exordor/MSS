...
usage() {
  cat <<'USAGE'
Usage: ./run_ros2_camera.sh [OPTIONS] [extra ros2 launch args]
...
start_ros2_healthcheck "camera" "galaxy_camera"

# Extract camera logs to separate file (runs in background)
# This creates app/galaxy_camera.log from filtered 00_master.log content
start_node_log_extractor "galaxy_camera" &
EXTRACTOR_PID=$!

if [[ ${#EXTRA_ARGS[@]} -gt 0 ]]; then
  printf '%q ' ros2 launch ros2_bringup camera.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} "${EXTRA_ARGS[@]}" > "${META_LOG_DIR}/launch_cmd.txt"
  ros2 launch ros2_bringup camera.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} "${EXTRA_ARGS[@]}"
else
  printf '%q ' ros2 launch ros2_bringup camera.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} > "${META_LOG_DIR}/launch_cmd.txt"
  ros2 launch ros2_bringup camera.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}}
fi
