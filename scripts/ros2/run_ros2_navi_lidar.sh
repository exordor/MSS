...
usage() {
  cat <<'USAGE'
Usage: ./run_ros2_navi_lidar.sh [OPTIONS] [extra ros2 launch args]
...
start_ros2_healthcheck "lidar" "navi_lidar_driver"

# Extract navi_lidar logs to separate file (runs in background)
# This creates app/navi_lidar_driver.log from filtered 00_master.log content
start_node_log_extractor "navi_lidar_driver" &
EXTRACTOR_PID=$!

cd "${WS_DIR}"
if [[ ${#EXTRA_ARGS[@]} -gt 0 ]]; then
  printf '%q ' ros2 launch ros2_bringup navi_lidar.launch.py "config:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} "${EXTRA_ARGS[@]}" > "${META_LOG_DIR}/launch_cmd.txt"
  ros2 launch ros2_bringup navi_lidar.launch.py "config:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} "${EXTRA_ARGS[@]}"
else
  printf '%q ' ros2 launch ros2_bringup navi_lidar.launch.py "config:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} > "${META_LOG_DIR}/launch_cmd.txt"
  ros2 launch ros2_bringup navi_lidar.launch.py "config:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}}
fi
