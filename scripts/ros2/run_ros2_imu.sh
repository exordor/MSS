...
usage() {
  cat <<'USAGE'
Usage: ./run_ros2_imu.sh [--config /path/to/sbg.yaml] [extra ros2 launch args]
...
if [[ ${#EXTRA_ARGS[@]} -gt 0 ]]; then
  ros2 launch ros2_bringup imu.launch.py "config_file:=${CONFIG_PATH}" "${EXTRA_ARGS[@]}"
else
  ros2 launch ros2_bringup imu.launch.py "config_file:=${CONFIG_PATH}"
fi
