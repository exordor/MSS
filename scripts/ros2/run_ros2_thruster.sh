...
usage() {
  cat <<'USAGE'
Usage: ./run_ros2_thruster.sh [OPTIONS] [extra ros2 launch args]
...
CONFIG_PATH="${CONFIG_OVERRIDE:-${THRUSTER_CONFIG:-${DEFAULT_CONFIG}}}"
...
start_ros2_healthcheck "thruster" "thruster_wifi_node"

cd "${WS_DIR}"
if [[ ${#EXTRA_ARGS[@]} -gt 0 ]]; then
  printf '%q ' ros2 launch ros2_bringup thruster.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} "${EXTRA_ARGS[@]}" > "${META_LOG_DIR}/launch_cmd.txt"
  ros2 launch ros2_bringup thruster.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} "${EXTRA_ARGS[@]}"
else
  printf '%q ' ros2 launch ros2_bringup thruster.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}} > "${META_LOG_DIR}/launch_cmd.txt"
  ros2 launch ros2_bringup thruster.launch.py "config_file:=${CONFIG_PATH}" ${LOG_LEVEL:+log_level:=${LOG_LEVEL}}
fi
